...
class PortfolioViewsTests(BaseTestCase):
    ...
    def test_home_page_loads(self):
        ...
        THEN check that status code is 200 and response data contains ' '
        """
...
class AuthTests(BaseTestCase):
    ...
    def test_logout_user(self):
        ...
        with self.client:
            self.client.post(
                url_for('auth.login'),
                data=dict(username='test1', password='test1test1'),
                follow_redirects = True
            )
...
class TodoTests(BaseTestCase):
    ...
    def test_delete_todo(self):
        ...
        with self.client:
            response_post = self.client.post( url_for('todo.todos'),
                data=data, 
                follow_redirects=True
            )
            response = self.client.post(
